      ...
      <div className="mx-auto max-w-2xl px-6">
        ...
        <div className="mt-10 space-y-2">
          {FAQ_ITEMS.map((item) => (
            <details
              key={item.question}
              className="faq-item group rounded-2xl border border-border bg-card px-6 py-4 transition-colors open:pb-5 hover:border-ring"
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-medium text-muted-foreground transition-colors group-open:text-foreground">
                {item.question}
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-border text-muted-foreground/70 transition-all duration-200 group-open:rotate-45 group-open:border-ring group-open:text-foreground">
